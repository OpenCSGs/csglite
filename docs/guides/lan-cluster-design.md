...
**分发。** `ggml-rpc-server` 随 `llama-cpp-assets` 一起分发，版本与 `llama-server`
严格一致——RPC 协议不保证跨版本兼容，混版本会在加载时崩溃或给出错误结果。

**安全。** 上游明确标注 proof-of-concept、fragile and insecure、
**"Never run the RPC server on an open network"**，且完全没有鉴权。因此
worker **只监听回环**，由集群传输层在已有的 mTLS 节点通道（11438）上做端口转发。
这正好补上它没有鉴权的缺陷：能连到 RPC 端口的前提是已经通过证书钉扎的 mTLS。
绝不直接对局域网暴露 50052。

**权重只在发起节点。** RPC 的工作方式是发起节点读 GGUF、把张量推给 worker，
worker 本地不需要模型文件。代价是首次加载要把约三分之二的权重推过网络：19 GB 的
模型推 12.7 GB 到两台，按 30 MB/s 约 7 分钟。worker 端必须开 `-c` 本地张量缓存
（默认 `$HOME/.cache/llama.cpp/rpc`），之后加载才不再重传。缓存要纳入磁盘配额与
清理策略。

**调度器。** 跨节点模型对调度器是一个**只存在于发起节点**的模型，不参与常规多实例
放置。但参与切分的节点必须按实际占用扣减可用显存，否则调度器会把它们当空闲机器再
派别的活，导致双方一起 OOM。这需要 `Status` 增加一个「被跨节点模型占用」的字段。

**失败处理。** 任何一个参与节点掉线，整个推理立刻失败，且**无法故障切换**——权重
分片在别的机器上，没有第二份。因此：请求返回明确错误而非重试；自动拆除整个跨节点
加载；在 UI 上把这类模型标记为「无冗余」。这与第 7.5 节的多实例故障切换是两套语义，
不能混用。

**接口。** 模型页增加「跨节点加载」入口，选择参与节点与切分比例（默认按可用显存
比例）。API 为 `POST /api/cluster/models/{model}/span`，返回一个 span 句柄；
`DELETE` 拆除。`source` 保持不变，请求照常发给发起节点。

**门控。** 跨节点加载属于 EE，社区版不开放——社区版上限两个节点，拆分收益本就有限，
且这是最容易被滥用来规避节点配额的路径。

### 10.5 判断与触发条件

**现在不做。** 当前的多实例路由解决的是绝大多数真实场景，而跨节点只在客户明确要跑
放不进单机的模型时才有价值，且要付 2.5 倍速度代价、失去故障切换、引入一个上游自称
不安全的组件。

**触发条件**：客户提出要跑 32B 及以上、并接受单请求速度显著低于单机。届时按 10.4
实施，验收标准是：32B Q4 在三台 16 GB 机器上能加载并稳定出词，首次加载时间与二次
加载时间分别记录，参与节点掉线时错误明确且资源被正确释放。
