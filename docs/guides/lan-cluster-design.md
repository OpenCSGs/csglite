...
4. **入口自增预留**：调度器把自己刚派发但遥测尚未反映的请求计入该节点的
   `active`，避免同一瞬间的并发请求全部压到同一节点（PAIR 同样处理）。这里必须
   是**当前在途**的请求数，不是历史累计值——累计值只增不减，刚启动的集群上人人为
   零、人人同分，一批相同请求就会全部落到稳定抖动选中的那一台（实测 36 个并发
   embedding 全部压在入口节点，另外两台空转）。
...
前九节讲的是**多实例**：每台机器各自完整加载模型，请求按预测完成时间分派。
它解决并发，实测三台把吞吐提到 2.68 倍、首字延迟减半（第 7 节）。

本节解决的是另一件事：**一个模型大到任何一台机器都装不下**。三台 16 GB 的
Mac，Metal 单机实际可分配约 12 GB（`llama-server --list-devices` 报
`12124 MiB`），合计约 36 GB。这决定了哪些模型靠拆分才够得着：

| 模型 | 量化 | 权重 | 单机 12 GB | 三机 36 GB |
| --- | --- | ---: | --- | --- |
| 32B | Q4_K_M | ~19 GB | 装不下 | 可以，留约 17 GB 给 KV 与运行时 |
| 32B | Q8 | ~35 GB | 装不下 | 勉强，KV 无余量，不建议 |
| 30B MoE (A3B) | Q4_K_M | ~18 GB | 装不下 | 可以，且每 token 只激活约 3B，速度远好于稠密 32B |
| 70B | Q4_K_M | ~40 GB | 装不下 | 不够，需要五台 |

没有拆分时，32B 在 12 GB 机器上只能靠磁盘换页，prima.cpp 论文测得
llama.cpp 单机 70B 是 10120 ms/token，即每 token 十秒量级，工程上等于不可用。
所以这里的对比不是「拆分 vs 单机」，而是「拆分 vs 跑不了」。

### 10.2 三条路线

**llama.cpp RPC 后端（推荐）。** 每台跑 `ggml-rpc-server`，发起节点用
`llama-server --rpc a:50052,b:50052` 把权重按可用显存比例切到多机。项目分发的
`llama-server` 已经编译进这个后端（`--rpc` 参数存在、`libggml-rpc.dylib` 随包），
缺的只是 worker 端可执行文件。跨平台、不挑硬件、与现有 GGUF 流程完全一致。

**MLX 分布式。** Apple 官方路线，流水线并行。前提是 **Thunderbolt 5 + RDMA**
（macOS 26.2 起，配 Apple 开源的 JACCL），两节点开销约 15–25%，明显优于 RPC。
但只支持 Apple Silicon，且离开 Thunderbolt 直连退回 Wi-Fi 后优势基本消失。
客户若是全 Mac 且愿意拉 Thunderbolt 线，值得单独立项。

**prima.cpp。** 学术原型，针对的正是「Wi-Fi、异构、内存不够」这个场景，用流水线环
并行加一个求解整数规划的调度器，70B 做到 674 ms/token，比 exo 与
distributed-llama 快 5–17 倍。思路值得借鉴，但不是可直接产品化的东西。

exo 按内存比例切层会让快设备闲置；distributed-llama 用张量并行，每 token 需
64 次以上 all-reduce、每次约 150 ms，论文直接判定对 Wi-Fi 集群不可行。两者都不选。

### 10.3 实测开销（三台真机，5 GHz Wi-Fi）

模型 Qwen3.5-2B F16（3.62 GB，三台都装得下，用来单独测量开销），官方构建 b11071：

| 配置 | 预填充 tok/s | 生成 tok/s | 相对单机 |
| --- | ---: | ---: | ---: |
| 单机 M5 | 2538 | 33.5 | 基准 |
| M5 + M4 | 857 | 18.4 | 0.55× |
| M5 + M4 + M1 Pro | 460 | 13.0 | 0.39× |
| 全部层放 M4，M5 只调度 | 709 | 14.1 | 0.42× |

后来用一组去掉「切分」这个变量的对照，把成本定位清楚了：同一个 2B、同一台机器
（node-be1af1），一次走普通集群路由原生跑、一次把权重推进它的 RPC worker 由另一
台驱动，**中间没有任何切分**：

| 同一台机器上的同一个模型 | 生成 tok/s | ms/token |
| --- | ---: | ---: |
| 原生（无 RPC） | 26.1 | 38 |
| 经 RPC 驱动 | 6.5 | 154 |
| 差额＝传输 | | ~116 |

**所以开销的主体是传输，不是切分。** 同期量到这条链路的 RTT 平均 23.5 ms（抖动
5–67 ms），每个 token 还要把 logits（词表约 15 万，f32 约 600 KB）取回发起节点。
27B 的两组数据从另一面印证了同一件事：放在一台远端机器上是 2.09/2.16 tok/s，分到
两台是 1.74–2.68 tok/s，**在误差范围内相同**——多一台机器几乎不改变什么。

由此还有一个调优改不动的上限：层与层前后依赖，B 机必须等 A 机算完，两台各读自己
那一半权重是**相加**而不是重叠。**跨机分载最好只能逼近「一台装得下的机器」的速度，
不可能超过它。** 要在单请求上超过单机只能做张量并行——每层一次 all-reduce，这正是
NVLink 与 InfiniBand 存在的理由，局域网换任何引擎都补不上。

换更快的链路（两台 Mac 之间的 Thunderbolt 直连，或 10 GbE 过交换机）能把传输这部分
成本几乎清零，值得做；但它**消不掉上面那个上限**。

**结论是它只应在装不下时启用，永远不要用它去加速一个已经装得下的模型。** 而且在
决定分载之前，通常有更划算的答案：换更小的量化、换同等质量但每 token 只激活一小
部分权重的 MoE、换一台内存更大的机器，或者把这个模型路由到 provider。四条都不成立
才轮到分载。

### 10.4 已实现的设计

**接口。** `POST /api/cluster/spans` 分载一个模型，`GET /api/cluster/spans` 看当前
分载，`DELETE /api/cluster/spans/{model...}` 拆除。请求体：

```json
{"model": "unsloth/Qwen3.8-27B-GGUF", "num_ctx": 4096,
 "nodes": ["<uuid>", "..."], "host_devices": false}
```

`nodes` 不填就选所有有余量的健康成员，按可用显存从多到少排序。模型分载后，普通的
`/v1/chat/completions` 照常发给承载节点，`source` 语义不变。分载创建受 EE 门控
（`feature.lite.cluster_model_span`），查看与拆除不受门控——License 过期时必须仍能
看到并拆掉已经建立的分载。

**承载节点默认不持有权重（`host_devices: false`）。** 这是实测踩出来的：llama.cpp
会按可用显存给发起机器分一份权重，然后把整个 KV cache 也放在它上面。装不下这个模型
的机器正好受不了这一下——加载报成功，之后每个请求都返回 compute error
（`kIOGPUCommandBufferCallbackErrorOutOfMemory`）。改成 `-dev RPC0,RPC1`
只用远端设备后，27B 正常出词。机器有余量时可以显式打开。

**加载成功不等于能用，所以先问一句。** 权重放不进给它的设备时，llama.cpp 报加载成功，
然后每个请求都返回 compute error。建一次分载要几分钟，所以加载完先要一个 token：
答不出来就地拆除并把原因返回，而不是把一个「已加载但坏了」的模型交出去。实测
`host_devices: true` 在没有余量的 M5 上正是这种表现：加了这道检查之前是 48 秒加载
成功、随后每个请求 500，之后是 53 秒返回 400 并说明原因。

**上下文必须显式限制（`num_ctx`）。** KV cache 是加在权重之上的，单机默认值足以
压垮一个本来放得下的分载。

**版本必须一致。** RPC 协议没有版本协商，而 `llama-server` 连不上 worker 时**不会
报错**：它打一行日志，然后把整个模型加载到本地。对装不下的模型，这就是「明确拒绝」
与「API 报成功、机器在磁盘上抖」的区别。因此加载前先比对两端 llama.cpp build，再
对隧道做一次端到端探活，任何一项不过就拒绝。

**安全。** worker 只监听回环，字节流全部走集群已有的 mTLS 通道（11438），按 UUID
钉扎证书。连得上 worker 的前提是已经配对成功。上游明确说它没有鉴权、不能暴露在网络
上——这里它根本没有暴露在网络上。

**权重只在发起节点，worker 端有缓存。** 首次加载要把权重推过网络：27B Q4
（16.5 GB）推到两台，首次约 8 分钟；worker 端 `-c` 缓存命中后 83 秒。缓存目录
由 `LLAMA_CACHE` 指定，落在节点自己的 `rpc-cache` 目录下。缓存在磁盘上，worker
进程重启不会丢，这也是下面敢于「用完就停 worker」的前提。

**分载的模型被钉在内存里。** 建一次要付几分钟的网络传输，普通的空闲卸载会把这笔账
转嫁给下一个提问的人。只有拆除分载时才卸载。

**一台机器同时只承载一份分载模型。** 第二份会向同一个 worker 要机器给不出的显存，
而它的回应是 `abort`，连第一份一起带走。所以由被问的那台机器自己回答「我已经在为
X 持有权重了」（409），而不是由发起方去猜一份可能已经过时的状态；拆除分载时释放是
同步的，因此拆完可以立刻重新分载，不必等任何一轮轮询。

**出借内存的机器不再接冷加载。** worker 持有的显存不属于该节点自己的任何模型，而且
在部分机器上 GPU 计数器根本看不到它（实测 M1 Pro 报 0.5 GB，M4 报 8.8 GB），所以
不能让调度器自己去推断。`Status` 增加 `span_worker` 字段，调度器在「需要冷加载」
这一支上直接排除它，已经加载好的模型照常服务，`explain` 会给出原因。

**没有冗余，而且明说。** 权重只有一份、分布在多台机器上，掉一台就没了。API 返回
`"redundant": false`；`spanWatchLoop` 每 20 秒检查一次参与成员，整体拆除并写明原因，
而不是让请求挂在永远不会到达的张量上。两种情况都算丢：机器掉线（实测杀掉一台后
60 秒内完成拆除，这 60 秒是成员被判定为 down 所需的时间），以及机器还在、里面的
worker 死了——后者由成员自己在 `span_worker` 字段里报告，但只有在本节点自分载开始
之后真正轮询到过它时才采信（用本机时钟判断，两台机器的时钟不是同一个）。

**内存一定要还回去。** 承载节点在卸载、进程退出、以及加载失败时都会通知参与成员释放；
参与成员这边还有两道自己的保险：持有者被移出集群时释放，以及 5 分钟完全没有张量流量
时释放（这是本机的一手证据，不依赖对方还活着的说法）。**不以健康状态判断**——刚重启
的节点尚未轮询过任何对端，把「还没问过」当成「已经没了」会在分载刚建好时就把内存收
回去，这个 bug 在三机实测中出现过一次。没有任何分载占用时 worker 立即停止：它持有的
是本机的显存，而让重载变快的缓存在磁盘上，留着进程只省一两秒却占着几个 GB。

**worker 自己死掉也要认。** `ggml-rpc-server` 在显存不够时是 `abort` 而不是返回错误。
所以：启动它的那个 goroutine 独占 `Wait()`，进程一退立刻清掉状态；复用之前先连一次
端口确认还活着，而不是把一个已经死掉的端口号交出去——实测出现过一次，表现为下一次
分载报「worker 刚连上就断开」。

**节点被强杀留下的孤儿 worker。** 被 `kill -9` 或断电的节点来不及停自己的子进程，
孤儿会一直占着一份不再存在的模型的显存。因此启动 worker 时把 pid 与二进制路径写进
`rpc-worker.pid`，下次启动时读回来，确认该 pid 仍然是同一个程序（`ps -o comm=`，
pid 会被复用）后再杀掉。Windows 上不做这件事，宁可放过也不误杀。

**分发。** `ggml-rpc-server` 与 `llama-server` 出自同一个 llama.cpp release 包，
`scripts/install.sh` 与 `install.ps1` 会一并安装到同一目录；查找时只看
`llama-server` 旁边，不走 PATH，避免混进版本不一致的副本。

### 10.5 验收结果（三台真机）

M5（承载）、M4、M1 Pro，5 GHz Wi-Fi，llama.cpp b11071，模型
`unsloth/Qwen3.8-27B-GGUF`（Q4_K_M，16.5 GB）。

| 场景 | 结果 |
| --- | --- |
| 单机加载 27B | HTTP 500 compute error，确认单机跑不了 |
| 分载到两台远端机器 | 首次 8 分钟，worker 缓存命中后 83 秒 |
| 分载后推理 | 200，64 tokens / 23.9s = 2.68 tok/s（多轮 1.74–2.68） |
| 同一个 27B 只放一台远端机器 | 2.09 / 2.16 tok/s，与分到两台在误差内相同 |
| 7702 token 长提示 | 200，答案正确，180s；跨机预填充约 47 tok/s |
| 流式输出 | 8 个数据块，首字 5.5s |
| 三个并发请求 | 全部 200，依次排队（分载模型只给一个 slot）|
| 空闲超过出借宽限期 | 仍然加载着，仍然能回答 |
| 集群里可见 | 承载节点列出分载，参与节点标记出借内存 |
| 出借机器被保护 | 冷加载被排除并给出原因；已加载的 embedding 模型仍在三台间分担 32 / 11 / 5 |
| 重复分载 | 400，明确说已经分载 |
| 不存在的模型 | 400，明确说本地没有 |
| 指定到 drain 节点 | 拒绝并说明该节点不接活 |
| 拆除分载 | 出借标记立即清除，机器恢复可用 |
| 参与成员被杀 | 40–60 秒内整体拆除，日志写明原因 |
| 成员恢复 | 集群自愈，可以重新分载并推理 |
| 多实例回归 | 15 个场景全部通过，未受影响 |
...
2. ~~**`ee/LICENSE` 补充条款**~~：已落地。`ee/LICENSE` 顶部增加了「社区版
   附加授权」，明确未授权用户可在软件自身强制执行的限额内将功能用于生产，
   局域网集群的社区限额为每集群两个节点。措辞仍需法务复核后定稿。
