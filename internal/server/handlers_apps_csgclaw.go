...
func (s *Server) saveCSGClawModel(ctx context.Context, modelID, modelSource string) error {
	...
	if err := s.configureCSGClaw(ctx, binary, resolvedModel, resolvedSource, modelIDs, true); err != nil {
		return err
	}
...
func (s *Server) configureCSGClaw(ctx context.Context, binary, modelID, modelSource string, modelIDs []string, forceRestartManager bool) error {
	...
	if forceRestartManager || csgclawConfigNeedsManagerRecreate(modelBaseURL, apiKey, modelID) {
		log.Printf("AI APP csgclaw: manager/config drift detected; stop manager before synced config")
		stopCSGClawManager(ctx, binary)
	}
	if err := ensureCSGClawManagedConfig(modelBaseURL, apiKey, modelID, models); err != nil {
		return fmt.Errorf("writing CSGClaw config: %w", err)
	}

	log.Printf("AI APP csgclaw: config synced base_url=%s models=%s", modelBaseURL, strings.Join(models, ","))
	return nil
}

func ensureCSGClawManagedConfig(baseURL, apiKey, modelID string, models []string) error {
	...
	if strings.TrimSpace(input) == "" {
		input = defaultCSGClawConfig()
	}

	updated := setCSGClawManagedModelConfig(input, baseURL, apiKey, modelID, models)
